...
test.describe("Skeleton Component", () => {
  test("should render with default props", async ({ mount }) => {
    ...
    await expect(component).toHaveClass(/skeleton--rectangle/);
    await expect(component).toHaveClass(/skeleton--pulse/);
  });

  test("should render text variant", async ({ mount }) => {
    const component = await mount(<Skeleton variant="text" />);
    await expect(component).toHaveClass(/skeleton--text/);
  });

  test("should render circle variant", async ({ mount }) => {
    const component = await mount(<Skeleton variant="circle" />);
    await expect(component).toHaveClass(/skeleton--circle/);
  });
  ...
  test("should apply wave animation", async ({ mount }) => {
    ...
    await expect(component).toHaveClass(/skeleton--wave/);
  });
  ...
  test("should have no animation class when animation is none", async ({
    ...
    await expect(component).not.toHaveClass(/skeleton--pulse/);
    await expect(component).not.toHaveClass(/skeleton--wave/);
  });
  ...
  test("should render multiple text lines as a group", async ({ mount }) => {
    ...
    await expect(component).toHaveClass(/skeleton__group/);
    const lines = component.locator(".skeleton--text");
