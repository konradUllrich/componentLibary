...
test.describe('UserAvatars Component', () => {
  ...
  test('should render different sizes', async ({ mount }) => {
    ...
    const avatars = component.locator('.mp-user-avatar');
    await expect(avatars.nth(0)).toHaveClass(/mp-user-avatar--sm/);
    await expect(avatars.nth(1)).toHaveClass(/mp-user-avatar--md/);
    await expect(avatars.nth(2)).toHaveClass(/mp-user-avatar--lg/);
  });
  ...
  test('should have correct BEM structure', async ({ mount, page }) => {
    ...
    await expect(page.locator('.mp-user-avatars')).toBeVisible();
    await expect(page.locator('.mp-user-avatars__group')).toBeVisible();
    await expect(page.locator('.mp-user-avatars__item')).toHaveCount(3); // 2 visible + 1 overflow
  });
  ...
  test('should forward ref to root element', async ({ mount, page }) => {
    ...
    const rootElement = page.locator('.mp-user-avatars');
