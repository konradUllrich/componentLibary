...
test.describe("Button Component", () => {
  ...
  test("should pass accessibility checks with aria-label", async ({
    mount,
    page,
  }) => {
    await mount(
  ...
  test.describe("Hover States", () => {
    test("should have hover CSS rules for all button variants", async ({
      mount,
      page,
    }) => {
      await mount(
      ...
      const hasHoverRules = await page.evaluate(() => {
        ...
        return {
          ...
          secondary: rules.some((r) =>
            r?.includes(".mp-button--secondary:hover"),
          ),
          destructive: rules.some((r) =>
            r?.includes(".mp-button--destructive:hover"),
          ),
    ...
    test("ghost button hover should have explicit color defined", async ({
      mount,
      page,
    }) => {
    ...
    test("disabled buttons should have disabled hover rules", async ({
      mount,
      page,
    }) => {
      ...
      const hasDisabledRules = await page.evaluate(() => {
        ...
        return rules.some(
          (r) => r?.includes(":hover") && r?.includes(":not(:disabled)"),
        );
      });
    ...
    test("ghost button should maintain foreground color in hover state", async ({
      mount,
      page,
    }) => {
      ...
      const colorConsistency = await page.evaluate(() => {
        ...
        const defaultRule = rules.find(
          (r) => r.selectorText === ".mp-button--ghost",
        );
        ...
        return (
          ...
          defaultColor.includes("--mp-color-foreground")
        );
      });
    ...
    test("ghost button hover should have background and border colors defined", async ({
      mount,
      page,
    }) => {
      await mount(<Button variant="ghost">Ghost Button</Button>);
