...
test.describe("HorizontalNav Component", () => {
  ...
  test("should render navigation items in desktop view", async ({ mount }) => {
    const component = await mount(<HorizontalNav items={mockNavItems} />);

    await expect(component).toBeVisible();
    // Note: component IS the nav element (root), no need for locator
  ...
  test("should render items with icons", async ({ mount, page }) => {
    ...
    await mount(<HorizontalNav items={itemsWithIcons} />);
  ...
  // Skip: Functions returning values cannot be properly serialized in Playwright CT
  // This includes event handlers like onClick. The function becomes undefined when
  // passed through the Playwright boundary, so preventDefault never executes.
  // See PLAYWRIGHT_CT_LIMITATIONS.md for details.
  test.skip("should call onClick handler when item is clicked", async ({
    mount,
    page,
  }) => {
    ...
    await mount(<HorizontalNav items={itemsWithHandlers} />);
  ...
  test("should use semantic nav element", async ({ mount }) => {
    ...
    // Component IS the nav element (root)
    await expect(component).toHaveRole("navigation");
  });
  ...
  // See PLAYWRIGHT_CT_LIMITATIONS.md for details.
  test.skip("should forward ref to nav element", async ({ mount }) => {
  ...
  test.describe("Mobile View", () => {
    ...
    test("should render mobile dropdown when viewport is narrow", async ({
      mount,
      page,
    }) => {
    ...
    test("should render desktop nav when viewport is wide", async ({
      mount,
      page,
    }) => {
    ...
    test("should render select with all items in mobile view", async ({
      mount,
      page,
    }) => {
      await page.setViewportSize({ width: 400, height: 800 });
    ...
    test("should select active item in mobile dropdown", async ({
      mount,
      page,
    }) => {
      await page.setViewportSize({ width: 400, height: 800 });
  ...
  test.describe("Keyboard Navigation", () => {
    test("should support Tab navigation between items", async ({
      mount,
      page,
    }) => {
      await mount(<HorizontalNav items={mockNavItems} />);
    ...
    // Skip: Pressing Enter on a link causes real navigation in Playwright CT,
    // which navigates to the href URL (e.g., /home). Since these pages don't exist
    // in the test environment, we get a 404 and lose the component. Navigation
    // behavior should be tested in E2E tests or with navigation mocking.
    test.skip("should support Enter key to activate links", async ({
      mount,
      page,
    }) => {
      await mount(<HorizontalNav items={mockNavItems} />);
  ...
  test.describe("Edge Cases", () => {
    test("should handle empty items array", async ({ mount }) => {
      const component = await mount(<HorizontalNav items={[]} />);
    ...
    test("should handle single item", async ({ mount, page }) => {
      ...
      await mount(<HorizontalNav items={singleItem} />);
    ...
    test("should handle items without href", async ({ mount, page }) => {
      ...
      await mount(<HorizontalNav items={itemsNoHref} />);

      await expect(page.locator("text=Item 1")).toBeVisible();
    });

    test("should handle items with no isActive flag", async ({
      mount,
      page,
    }) => {
      ...
      await mount(<HorizontalNav items={itemsNoActive} />);

      // Should still render without errors
      await expect(page.locator("text=Item 1")).toBeVisible();
    });

    test("should handle complex React nodes as labels", async ({
      mount,
      page,
    }) => {
      ...
      await mount(<HorizontalNav items={complexItems} />);

      await expect(page.locator("text=Bold")).toBeVisible();
      await expect(page.locator("text=Text")).toBeVisible();
    });

    test("should handle string labels in mobile select", async ({
      mount,
      page,
    }) => {
  ...
  test.describe("Active State", () => {
    test("should handle multiple active items gracefully", async ({
      mount,
      page,
    }) => {
      ...
      await mount(<HorizontalNav items={multipleActive} />);
    ...
    test("should default to first item when no active item in mobile", async ({
      mount,
      page,
    }) => {
      await page.setViewportSize({ width: 400, height: 800 });
