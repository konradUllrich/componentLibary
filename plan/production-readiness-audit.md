...
- **Co-located a `ComponentName.example.tsx` next to every component** in `common/`, `controls/`, `data-display/`, `layout/`, and `intrexx/` (46 components total). Each exports `usageSource` (a template string) and `UsageExample` (a live render of it); every demo page's "Usage" section now renders both instead of a hand-duplicated code string, and each component's JSDoc `@example` links to its `.example.tsx`. This also surfaced and fixed several real drift bugs along the way: 30 demo pages still importing from the old `@konradullrich/mp-components` package name, `AppLayout`'s example passing compound components as children instead of via its `header`/`sidebar` props (silently swallowed into the main slot), a stale `Datalist` JSDoc block documenting a `createColumns` signature that doesn't match the real one, and `TreeEditorPage`'s "Usage" section documenting an entirely fictional `SortableTree` API (the real export is `Tree`) that would have failed to import if copy-pasted.
